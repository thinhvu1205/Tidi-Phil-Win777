...
public class InfoPlayerInGame : BaseView
{
    ...
    async public void setInfo(Player _player, bool showActionButtons = true)
    {
        ...
        m_AddFriend.SetActive(Globals.User.userMain.VIP >= 2 && player.id != Globals.User.userMain.Userid && player.vip >= 2 && !Globals.COMMON_DATA.IdFriend.Contains(player.id) && !Globals.COMMON_DATA.IdRequestFriend.Contains(player.id) && !Globals.COMMON_DATA.IdInviteFriend.Contains(player.id));
